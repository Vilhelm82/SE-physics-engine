...
Verification: **17 exact checks pass** in [cella_horizon_reuse.py](/home/williaml/seated-root/cella_horizon_reuse.py), with results in [cella-horizon-reuse-checks.json](/home/williaml/seated-root/docs/cella-horizon-reuse-checks.json). They check the coupling determinant and isometry, the gauge-map determinant, the signed curvature identity against direct Christoffel contraction for arbitrary diagonal functions in dimensions 2, 3 and 4, the existing load witness and signed reflection coefficient, and the area/normal identities. Cella's own weighted-jet replay was rerun and all six assertions passed.
