...
1. **Dark-space orientation gates:** strongest immediate native construction. Equation (5) is proved for the full declared fibre; (6) gives a measurable ideal gate. The linked finite-duration, composite, reference-access and compression extensions supply corrected gates, full response and resource costs. The immediate control target is the remaining second-order logical detuning rotation, with return index and duration chosen together. The occupation-dependent spectrum (7) is the first many-body extension test.
