...
The loop time is $9\tau_1=54.7530/a$, before any dump time. This is a substantial resource opportunity if heralded loss is acceptable. The existing SPLIT-1 runner still gives conditional infidelity $2.34\times10^{-8}$ at $\epsilon=10^{-3},\Delta/a=10^{-4}$ for its declared site detuning, so the useful continuation is the projected detuning-response calculation together with a physical finite-duration dump. For a stationary code with $HP=0$, ideal selective absorption has no-jump map $P+e^{-\kappa t_d/2}V_QQ$, where $V_Q$ is the bright-sector unitary; its surviving bright amplitude and duration can therefore be included explicitly. Neither finite dump performance nor a shorter corrected detuning word is established here.
