...
Inputs: [CP-1–4](2026-09-05-reflection-loop-composite-control.md), [RE-1–6](2026-09-05-reflection-loop-reference-echo.md), and the native [REALFIBER proof](</home/williaml/Cella Framework/Papers_Library/02_theorems_and_lemmas/geometric_fault_localization_and_decomposition/REALFIBER_THEOREM.md>). Controls and perturbations have angular-frequency units. $P=\operatorname{span}(d,r)$, $Q=\operatorname{span}(e_1,e_2)$, and $G=\operatorname{diag}(-1,1)|_P$.
...
![Duration, exposure, complete joint error, and finite-frequency reference response](reflection-loop-compression-tradeoffs.png)
...
[Runner](../reflection_loop_compression.py) · [full matrices, certificate and checks](reflection-loop-compression-checks.json) · [dependency ledger](EXTERNAL-MATHEMATICS-DEBT.md).
...
**Next mathematical target:** optimize within the response conditions (4)–(8), using the full detuning matrix and temporal Gram matrix alongside duration and exposure. The largest remaining coefficient is now explicit: reduce the second-order logical detuning rotation, with $n$ and the available duration chosen jointly. The present eleven-block member is a certified feasible point for that problem.
