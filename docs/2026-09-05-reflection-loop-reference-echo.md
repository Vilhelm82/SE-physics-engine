...
**Continuation:** [CM-1–5](2026-09-05-reflection-loop-compression.md) supplies a certified eleven-block construction with 49.484% of this echo's duration and exposure at equal return index, retaining the stated cancellation orders and giving its complete changed response coefficients. It also evaluates this echo at \(n=2,3\), derives linear gain-drift cancellation, and compares finite-frequency noise. The next target is reducing the remaining second-order logical detuning rotation while choosing \(n\) and duration jointly.
