...
The target left by [CM-1–5](2026-09-05-reflection-loop-compression.md) is closed in two forms:
...
Inputs: [finite returns](2026-09-05-reflection-loop-finite-duration.md), [composite control](2026-09-05-reflection-loop-composite-control.md), [reference echo](2026-09-05-reflection-loop-reference-echo.md), CM-1–5, and the native [REALFIBER proof](</home/williaml/Cella Framework/Papers_Library/02_theorems_and_lemmas/geometric_fault_localization_and_decomposition/REALFIBER_THEOREM.md>). The live Cella DAG's GFL:thm:realfiber node and canonical proof O1–O7 were checked. No new external search or import was used; finite-control composition remains retained EXT-018.
...
Fix $w_1=w_2=1,\ w_6=1.07$. Equations (7), (9) then form ten equations for ten free variables. [The control file](reflection-loop-detuning-controls.json) specifies certified centers and fixed coordinates for $n=1,2,3$, at $\nu/a^2=10$. Their total stretch weights are respectively
...
![Duration, exposure, pure detuning order, finite-error loss choices and reference drift response](reflection-loop-detuning-tradeoffs.png)

## SD-6. Replay and continuation

[Response runner](../reflection_loop_detuning_order.py) · [interval certificate](../reflection_loop_detuning_certificate.py) · [controls](reflection-loop-detuning-controls.json) · [105-check receipt and full matrices](reflection-loop-detuning-checks.json) · [dependency ledger](EXTERNAL-MATHEMATICS-DEBT.md).
...
**Next target:** cancel the short construction's remaining $QP$ detuning coefficient while controlling its gain coefficient. Derive the independent leakage constraints from the native symmetries, then solve them with duration and exposure in the objective. A concrete resource target is full second-order correction within the prior echo's $W=24$ duration multiplier, compared against the exact 33-block fallback above. No nonexistence or global optimum has yet been established for shorter words.
