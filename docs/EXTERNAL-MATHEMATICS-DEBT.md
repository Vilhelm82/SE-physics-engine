# External mathematics and first-principles debt

Maintained from 5 September 2026 at Will's instruction: record each online search and mathematical import, its actual use, and any remaining native construction.

**Current policy, 5 September 2026:** retain strong standard mathematics that is independently reproducible from its stated first principles. No rederivation is required merely to remove an external label. Prioritize extensions of Cella that complete a mathematical construction beyond the immediate application. Purely external items are deferred unless that extension work supplies a native route. Provenance remains after completion.

A completed mathematical construction and the model data it takes as input are recorded separately. Exact checks verify applications of the proved construction; they do not select those physical inputs.

This initial backfill covers sources cited in the current horizon, comparison and foundation-upgrade documents, together with the searches made during the pinch/load continuation. Earlier query wording that is unavailable is marked as such rather than reconstructed.

**Cella reconciliation, 5 September 2026:** the earlier corpus search was insufficient. The [source inspection and derived extensions](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-dependency-reconciliation.md) identify existing Cella inputs for the entries below. William has confirmed that the cited DIS chapters are legitimate physics research in Cella, separate from DIS-Engine: their response, storage and memory programme supplies an internal constitutive direction to develop; the curvature calculation can use Cella's directional channels; the area response can use its trace-decomposition mechanism. No new online sources were used for this reconciliation.

## Completed Cella extensions, 5 September 2026

- [Implicit normal geometry](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-normal-geometry.md): arbitrary nondegenerate codimension and fixed signature; full second fundamental form, normal connection/curvature, mixed invariants, defining-function gauge, and local area/trapping readout. Extends the mean-curvature and three-channel sources.
- [Regular frame transport](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-frame-transport.md): complete defining-function gauge quotient \((P,T)\), canonical edge map \(\bar\nu\mapsto U\bar\nu\), invertible full-shape encoding in XLINK-1's trine, smooth resolved transport, compatible connection/soldering family, torsion and exact integration conditions.
- [Tensor and finite-jet calculus](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-tensor-valuation.md): arbitrary off-diagonal metric, full curvature and finite covariant derivatives, determinant cancellations, certified jet depth and actual tensor pullback. Extends Local Curvature Calculus and LEAD7, including the computational target in DBP:gap:II4.
- [Constitutive divisors](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-constitutive-divisors.md): complete passive classes on intersecting faces, nonlinear integrability, geometric area power, variable-constitution exchange, weighted response lift and its descent obstruction. A constant resolved response generates the existing finite-time native crossing.
- [Minimum predictive state](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-predictive-state.md): exact polynomial dynamic-equivalence construction, response/deck descent and material-memory quotients. For the existing constant resolved law, fixed coefficients and both original effort channels, the lapse requires precisely `(xi,d)` at fixed `a`; an explicit finite derivative reconstruction proves necessity through the pinch. The full constant reciprocal passive family includes a rank-one case and a valid folded quotient. Restricted inputs and unknown material constants have separately proved counts.
- [Response on branched, resolved state spaces](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-response-on-branched-resolved-state-spaces.md): complete finite-dimensional passive linear relations and smooth graph classes, including changes of effort-domain rank; exact power transformation, singular lifting, mixed one-sided/two-sided divisor laws, network elimination and predictive descent. HG-1's ruler swap extends smoothly through the retained pinch and gives a new fixed-stratum obstruction under swap-invariant response and drive.

- [Quantum application extension](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-quantum-prediction-targets.md): full O(m−2) dark-space holonomy of Cella's real fixed-spectrum rank-two coupling family, an explicit orientation-reversing loop, and an occupation-dependent spin-spectrum continuation. Critical quantum memory and operator-response lifting are the next quantum theorem targets. EXT-010–015 record the retained quantum realization and comparison literature.
...
- **Source:** [Fontanella, arXiv:2211.03861](https://arxiv.org/pdf/2211.03861), equations (1.3)–(1.5).
- **Entry point:** [horizon-crossing derivation](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-horizon-crossing-metric.md), section 3; inherited by the subsequent pinch metric continuation.
- **Imported content:** the established null-coordinate framework used to identify the constructed metric family and its relation to horizon geometry.
- **Already rederived conditionally:** the regular null partner, radial Gram block, metric determinant and inverse follow explicitly from the native form plus a declared soldering map. The cited coordinate theorem is not needed to prove these algebraic identities.
- **Completed Cella extension:** [frame transport](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-frame-transport.md) combines the source gauge law with XLINK-1's realization. An explicit regular \(R\) gives \(\widehat G=R^T\eta R\), \(U=R^{-1}J\), and all compatible pairs \(\Omega=R^{-1}\omega R+R^{-1}\mathrm dR,\ \mathsf E=R^{-1}\vartheta\). Curvature, torsion, passive basis covariance and exact local integrability conditions are supplied. Both trapping and expanding completions of the same native state path are explicit.
- **Completed \(\nu\)-map:** the complete regular Euclidean second-jet quotient is \((P,T)\), where \(T_{ijk}=g_i(PHP)_{jk}/(g^Tg)\). The canonical edge extraction \(\bar\nu=(T_{123},T_{213},T_{312})\) and its native image \(U\bar\nu\) are invariant under every nonzero defining-function gauge. The full tensor obeys orthogonal-axis covariance; the three extracted components and their \(B\)-norm need specified role axes. Their physical/native attachment remains part of the coframe selection.
- **Full-shape continuation:** for every self-adjoint shape on a positive two-dimensional cut, the three trine directional curvatures \(\chi(S)\) recover the entire operator and satisfy \(\chi(S)^TB\chi(S)=-3\det S\). Tangent-frame changes induce an exact \(B\)-preserving action fixing the trace/clock. This extends to the full normal pencil \(n\mapsto S_n\) and its mixed determinant invariants. It supplies a covariant full-shape alternative to ambient edge extraction; the physical coupling to native load data remains to be chosen.
- **Remaining model target:** select \(\vartheta\), its clock/ruler attachment and its evolution from the native material law. With those data, the transport and soldering construction is available; native Gram data alone leave the displayed family.
- **Status:** REGULAR REPRESENTATIVES, TRANSPORT AND SOLDERING CLASS CONSTRUCTED; PHYSICAL COFRAME SELECTION OPEN. Standard null-coordinate geometry retained.

## EXT-002 — Null expansions and future outer trapping horizons

- **Source:** [Hayward, General Laws of Black-Hole Dynamics](https://arxiv.org/abs/gr-qc/9303006).
- **Entry point:** [rest-frame/trapping comparison](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-rest-frame-trapping-equivalence.md), sections 1–3; [load/transport continuation](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-pinch-load-transport.md).
- **Imported content:** the GR comparison criterion based on outgoing/incoming null expansions and the outer derivative condition.
- **Already rederived conditionally:** the expansion bridge, its regular coincidence criterion, and the explicit examples are calculated from the declared metric and area variation.
- **Completed Cella extension:** [implicit normal geometry](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-normal-geometry.md) reconstructs the full normal-valued second fundamental form from a cut's defining-function two-jet and ambient metric one-jet. Its normal traces calculate both null expansions, shear and the causal strata, with the full conormal gauge law. This is a local area calculation; an integrated area gives only an average. The native criterion remains \(\theta_L=D+\rho F/2\), with regular oriented coincidence precisely under its stated \(D=F\Xi\) and sign conditions.
- **Remaining model target:** select the physical cut and propagation law. For a proposed law, apply the explicit screen-integrability test and reconstructed expansions; their calculation is complete without importing an identification of the two horizons.
- **Status:** FULL CUT/EXPANSION CALCULUS CONSTRUCTED; PHYSICAL CUT AND PROPAGATION SELECTION OPEN. GR comparison criterion retained.

## EXT-003 — Dual mean curvature and the area-preserving normal

- **Source:** [Anco, Mean curvature flow and quasilocal mass for two-surfaces in Hamiltonian General Relativity](https://arxiv.org/abs/gr-qc/0402057).
- **Entry point:** [comparison theorem](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-rest-frame-trapping-equivalence.md), section 2.
- **Imported content:** interpretation of the dual mean-curvature vector as the area-preserving normal and its causal relation to marginal surfaces.
- **Already rederived conditionally:** uniqueness of \(K_{\rm area}=K-(\theta_K/\rho)J\), its norm and its relation to \(Z\) have direct proofs from the two-dimensional normal-plane pairings and the definition of area expansion.
- **Completed Cella extension:** the [normal-geometry construction](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-normal-geometry.md) supplies the area covector, its dual and kernel directly from the implicit cut jets. It includes arbitrary codimension, complete mixed shape invariants and normal curvature; the codimension-two application covers spacelike, timelike, nonzero null and zero mean vector. At zero mean every normal is area-preserving, so there is no selected line. The [constitutive extension](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-constitutive-divisors.md) pulls local area work back to state effort and includes spatial/label drift.
- **Remaining model target:** supply the physical area density/cut evolution and use its computed \(D=\theta_K\) to determine agreement or separation. This is the same cut-selection datum as EXT-002, not another unresolved normal-plane identity.
- **Status:** NORMAL AND AREA-POWER CONSTRUCTIONS COMPLETE FOR THE DECLARED CUT DATA; MODEL AREA EVOLUTION OPEN. Standard mean-curvature geometry retained.

## EXT-004 — Passive effort/flow response and dissipative Hamiltonian systems

- **Source:** [Bartel et al., Port-Hamiltonian Systems Modelling in Electrical Engineering](https://arxiv.org/html/2301.02024v1), section 1.
- **Entry point:** [load/transport continuation](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-pinch-load-transport.md), passive-response theorem and illustrative relaxation.
- **Imported content:** the effort/flow power pairing and the decomposition into a symmetric positive-semidefinite dissipative part and a skew part. The current work uses a declared local linear response; it does not assume an entire port-Hamiltonian model has been derived.
- **Additional retained background, no new online source:** finite-dimensional maximal monotone linear relations, their linear scattering/Cayley coordinates, generalized inverses and Schur elimination. These are standard tools, not attributed to the Bartel passage or claimed as new theory. The [general response construction](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-response-on-branched-resolved-state-spaces.md), BR-1/2/6/7, states and proves the exact forms used, including the rank-changing and constraint-port cases.
- **Already rederived conditionally:** the power identity, the passive horizon-response factorization, and the explicit relaxation energy inequality are proved directly in the continuation.
- **Completed Cella extension:** [constitutive divisors](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-constitutive-divisors.md) continues DIS chapters 10/12 and Cella's weighted divisor calculus. For arbitrary output/face incidence, all passive linear laws have dissipative factors \(R_{ij}=q_iq_j\bar R_{ij}\), while skew entries need the union of the faces. Nonlinear incrementally passive laws additionally satisfy the exact effort-Jacobian integrability condition. Area effort is derived by geometric pullback, with offsets accounted for; the variable-constitution momentum and storage balance is explicit. Smooth weighted response lift imposes stronger factors on both parts.
- **Immediate native result:** a constant positive resolved matrix yields \(d=T,\xi=1+kT,x=T+kT^2\) under a constant original effort. A response with nonzero transverse speed cannot also descend continuously to the collapsed point across an open range of directions, for finite original efforts. KIN-1's kinetic forms remain cost-side input; they do not by themselves supply a constrained-output mobility.
- **General continuation completed:** BR-4 classifies smooth passive laws at mixed boundaries by positive root-cover coefficients with exact parity. BR-3 supplies the adjugate ideal criterion for a general square response lift. BR-7 supplies the singular graph/constraint classification and smooth internal-solution module criterion; its exact passive example shows that elimination and specialization need not commute. BR-8 separates a vanishing response output from a normal barrier. All retain the actual effort domain, anchor and drive.
- **Remaining model target:** specify the actual DIS material coefficients, exchange/drive and admissible state/history dependence; couple them to the physical cut/coframe. Passivity and incremental passivity define the proved classes, rather than conclusions about every possible native material.
- **RH-1 simplification, 6 September 2026:** [constraint-surface section 6](2026-09-06-RH-1-cella-constraint-surface.md#6-recommended-simplification--one-restoring-coordinate) pulls the declared rank-one angle response back exactly to `s_dot=-eta*U_s`, `U=k*f(s,lambda)^2/2`. At fixed environment `U_dot=-eta*U_s^2`. Bartel section 1 was re-read; no new framework or physical coupling was adopted. The channel Hessian is not needed to evolve this reduced law. This is a recommendation and conditional reduction, not an implemented restoring device.
- **Status:** PASSIVE DIVISOR, NONLINEAR AND RESOLVED-RESPONSE CONSTRUCTIONS COMPLETE; ACTUAL MATERIAL/COFRAME LAW OPEN. Standard power and Hamiltonian accounting retained.

## EXT-005 — Raychaudhuri/optical evolution and curvature transport

- **Source:** [Hayward, General Laws of Black-Hole Dynamics](https://arxiv.org/pdf/gr-qc/9303006), null focusing equations; standard Levi-Civita curvature machinery.
- **Entry point:** [load/transport continuation](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-pinch-load-transport.md), focusing compatibility and the explicit spacetime witness.
- **Imported content:** pregeodesic, twist-free null congruence evolution; the metric-compatible torsion-free connection used to calculate Ricci curvature.
- **Already rederived conditionally:** the optical trace identity is derived from the curvature commutator under the stated geometric hypotheses. The witness's full Ricci tensor and focusing balance are independently calculated from its metric.
- **Completed Cella extension:** [frame transport](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-frame-transport.md) gives every compatible connection and its curvature/torsion; a coframe plus zero torsion uniquely fixes the Levi-Civita member. [Tensor valuation](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-tensor-valuation.md) extends Cella to arbitrary off-diagonal metric entries, every fixed signature, full Riemann curvature and all finite differential contractions. With \(\Delta=\det g\), the polynomial construction gives \(R_{ijkl}=N_{ijkl}/\Delta\) and the complete covariant-derivative recurrence. Weighted numerator and determinant cancellations are both resolved. Full tensor components retain information absent from scalar contractions.
- **Remaining model target:** select the coframe/attachment and whether physical transport has zero torsion; specify the propagated congruence. The compatible connection, curvature calculator and existing optical identity then apply under their stated hypotheses. A Cella role operation requires its actual metric pullback before invoking coordinate covariance.
- **Status:** COMPATIBLE TRANSPORT CLASS AND FULL CURVATURE CALCULUS CONSTRUCTED; PHYSICAL CONNECTION/CONGRUENCE SELECTION OPEN. Standard curvature and optical machinery retained.

## EXT-006 — Premetric constitutive reconstruction

- **Source:** [Hehl, Axion and dilaton + metric emerge from local and linear electrodynamics](https://arxiv.org/abs/1601.00320).
- **Entry point:** [foundation-upgrade programme](/home/williaml/seated-root/docs/lorentzian-foundation-upgrade-programme-2026-09-05.md), electromagnetic continuation.
- **Imported content:** a proposed route from a general constitutive response to characteristics and metric reconstruction under explicit restrictions.
- **Use so far:** a research recommendation; its reconstruction result has not been inserted into the pinch evolution proofs.
- **Cella checked and reusable:** the DIS response/energy programme is legitimate Cella physics research providing an internal constitutive direction; Cella's coupling form provides a separate algebraic input. Neither has yet been mapped to the native field-response operator. Investigate those maps before adopting the proposed external route. First derive the native constitutive operator and its propagation equations, then calculate their principal symbol and characteristics. A completed native characteristic/metric reconstruction theorem was not located in the inspected sources. Neither the edge form's signature nor the proposed DIS Hamiltonian establishes that reconstruction by itself.
- **Target if resumed:** obtain the native field-response operator and its principal symbol, then assess the strongest applicable reconstruction theorem against its actual hypotheses. The present finite-dimensional constitutive extension supplies no field principal symbol by itself.
- **Status:** DEFERRED PURELY EXTERNAL RESEARCH ROUTE; NOT ADOPTED IN THE PINCH PROOF. No reconstruction ceremony scheduled.

## EXT-007 — Smooth division, integrability and ODE uniqueness

- **Source:** standard mathematical background, used without a new online source: Hadamard smooth division, Frobenius integrability, local uniqueness for locally Lipschitz ODEs, local submersion charts/sections, and descent of a smooth even function through `r=t^2` to a smooth function on a half-space (Taylor remainder with parameters).
- **Entry point:** regular coincidence theorem, screen/cut existence condition, smooth pinch-flow lift, finite-time/invariant-pinch distinction, and the constitutive-state reduction criterion in reconciliation section 4.
- **Already rederived conditionally:** the division step is given by its integral formula. The new normal-plane and response identities do not depend on numerical fitting.
- **Completed Cella extension:** the [weighted tensor calculus](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-tensor-valuation.md) proves a requested-output-dependent finite jet bound, separates numerator/determinant front cancellations and transports the actual approach path under chart changes. It explicitly distinguishes a vanished finite jet from an identically zero smooth germ. [Frame transport](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-frame-transport.md) supplies exact local coframe and screen integration conditions. [Constitutive divisors](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-cella-constitutive-divisors.md) supplies the smooth normal-crossing class, nonlinear effort integration and weighted response-lift criterion.
- **Resolved-response extension:** [BR-3/4/5/8/9](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-response-on-branched-resolved-state-spaces.md) use smooth ideal divisibility and the even-function lemma for the complete mixed-boundary class, then uniqueness for invariant strata and crossing. HG-1 supplies the ruler exchange; its smooth extension fixes the positive exceptional set. Finite arrival there is excluded for a swap-equivariant locally Lipschitz field under swap-invariant drive. No such obstruction is asserted for the earlier orientation-sensitive input.
...
## EXT-009 — Finite-cover square classes and inertia

- **Source:** retained Kummer theory as explicitly used in Cella's [Conjugate Kummer Modules and Wreath-Product Closures](</home/williaml/Cella Framework/Papers_Library/02_theorems_and_lemmas/galois_horizon_and_kummer_covers/KUMMER_MODULE_WREATH_LIFT_THEOREM_2026-07-10.md>), sections 2–3 and 7; the general conjugate-Kummer criterion and normalized-incidence/special-fibre sources are linked in the response manuscript. No new online source was consulted.
- **Exact use:** [general response theory](/home/williaml/seated-root/docs/results/2026-09-05/2026-09-05-response-on-branched-resolved-state-spaces.md), equation (23), retains the actual square-relation quotient `W=V/R_rel`, its dual deck kernel, and valuation parity as the inertia character. This algebraic action supplies the branch data on which the response representation is defined.
- **Hypotheses:** finite separable base extension, its normal closure, finitely many nonzero conjugate radicands, characteristic different from two; for the displayed local inertia description, a discrete valuation with residue characteristic different from two and its henselian localization. Relations among radicands must be retained. A maximal wreath group and real realizability of algebraic loops are not inferred.
...
- **Compression and temporal continuation:** [CM-1–5](2026-09-05-reflection-loop-compression.md) derives circular moment conditions for arbitrary admissible primitive trajectories and certifies an exact eleven-block member by interval contraction. At equal return index it uses 49.484% of the nested echo's duration/exposure, retaining the six static-noise cancellations, mixed response, gain-phase order and first-order erasure. Its complete quartic response exposes improved gain and increased residual detuning coefficients. The continuation derives linear gain-drift cancellation and the temporal response Gram matrix, and evaluates both constructions at return indices 1, 2 and 3.
- **Second-order continuation:** [SD-1–6](2026-09-05-reflection-loop-second-order-detuning.md) closes the logical detuning target with certified thirteen-block controls at \(n=1,2,3\), \(\nu/a^2=10\). Native time-reflection symmetry reduces the complete logical target to two real equations. A finite Fourier algebra computes response at arbitrary order. A separate forward/inverse/forward construction cancels the full quadratic static-noise logarithm, preserves the parent's gain-only gate exactly, and raises reference/gain frequency-response orders to four/six. Its cubic response and extra duration/exposure are explicit; finite-error comparisons select the construction and return index under a stated resource budget.
- **Complete encoded continuation:** [SC-1–6](2026-09-05-reflection-loop-complete-code-correction.md) derives the real dimension of the first surviving encoded response and a general gain-order doubling theorem for primitives with a fixed reference. Certified 17-, 19- and 21-block controls at return indices 1, 2 and 3 cancel the complete second-order encoded gain/reference-detuning response within the echo's total stretch $W=24$. Pure-gain infidelity begins at eighth order. The bivariate finite Fourier recurrence supplies the complete leading joint sixth-order infidelity and extends to arbitrary joint order.
- **Native source check for the continuation:** queried the live Cella DAG for REALFIBER, found GFL:thm:realfiber, and read its canonical REALFIBER_THEOREM.md proof, O1–O7. Existing CP/RE derivations supply the primitive and response machinery. This source check was repeated for SD-1–6 and SC-1–6. No further online search or external mathematical import was made for CM-1–5, SD-1–6 or SC-1–6. Local SciPy constrained searches located the SC candidate centers; existence is established separately by interval contraction on a closed cube, with the maps and bounds supplied explicitly.
- **Scope and remaining work:** SC cancels full second-order encoded gain/reference response and retains first-order correction for the six static-noise generators. SD's raised construction retains the broader whole-space quadratic noise-log cancellation and higher temporal orders. SC's remaining affine reference drift has five independent real coordinates, giving a twenty-equation continuation when added to its fifteen static/gain equations. All comparisons require the stated repeatable controls and noise model; global optimality, historical originality and hardware advantage remain separate questions.
- **Endpoint-erasure comparison:** [SPLIT-1](2026-09-05-CLAIRE-split-loop.md) and its runner supply the native endpoint-projection construction. SC-7 of the linked continuation derives its exact two-dimensional no-flag map from those controls and the RF moving-frame propagator, showing eighth-order conditional gain infidelity and quadratic erasure.
- **Finite-dump extension:** [FD-1–6](2026-09-05-reflection-loop-finite-dumps.md) derives an exact five-loop control with inverse stretch $3\sqrt3/4$. It cancels first-order logical response to both code-site detunings and all four bright-sector generators at every native return, including the persistent detuning during finite dumps. The ideal projected gain map equals a scalar times a conjugate of the original trine; the actual finite map retains its small missed-bright floor. Full joint coefficients, the complete flag instrument, positive-rate absorbing evolution, duration and the distinct endpoint/transport loss laws are supplied. Remaining work is quadratic correction of the full instrument, including mixed-error flag dependence.
- **Native provenance for FD:** queried GFL:thm:realfiber and read canonical REALFIBER_THEOREM.md O1–O7, then extended local RF, SPLIT-1 and SC-7. Local linear-program searches suggested the exact angles and stretch; the universal response cancellation is proved by symbolic matrix identities with arbitrary primitive response coefficients. SciPy integrates the declared laboratory evolution; independent nominal/Fourier formulas and chronological dump integration check it. No online search, external pulse schedule or new external mathematical result was adopted.
...
## EXT-023 — RH-1 common-angle feedback and a declared reference sensor

- **Native basis checked:** reuse EXT-022's live DAG/backing REALFIBER, RF, CP, FD and SPLIT sources and the already inspected DIS chapters 5/10/12. Re-read the native code-plane rotation, actual RH-1 propagator/instrument, and the predictive-state construction's section 1. The existing angle is an actuator; retained command/environment state fits the explicit-state bookkeeping. These sources do not supply the new X-reference measurement or a passive restoring force. No corpus-wide absence claim is made.
- **Retained standard framework and source:** [Vepsäläinen et al., Improving qubit coherence using closed-loop feedback (2022)](https://www.nature.com/articles/s41467-022-29287-4), introduction, Results through equations (1)–(2), and sampling-bandwidth discussion. Reuse the established distinction between reference probing, feedback update and computational operation; finite sampling and quasi-static drift constrain bandwidth. No experimental performance, device control or Ramsey pulse construction is transferred to the native word. Standard scalar discrete-time feedback stability and independent Bernoulli statistics are retained.
- **Exact local use:** [the gimbal assessment](2026-09-06-RH-1-opportunities.md#a-gimballed-angle-feedback-feasibility) derives a paired known-input X contrast and `s_next=s-g*m_hat/4` using the native reflection response. The scalar recurrence, noise floor and drift lag follow from the stated sensor likelihood and local unitary approximation. This extends the angle pilot into an explicitly stateful controller, without a supplied true detuning or fidelity oracle. A standard bounded scalar numerical minimization supplies a stronger fixed-angle comparison with access to future plateaus; it supplies no global optimality certificate and no information to the controller.
- **Assumptions and open physical construction:** known d/r reference preparations, accepted-code X measurement with a fixed basis, common reference/data drift, independently repeated trials, detuning held through each entire batch, and sufficient actuator speed between words. Reference hardware, preparation/readout error, physical drift spectrum, actuator transitions, additional latency and decoherence are unmodeled. Local stability is not global acquisition or within-word motion. General SU(2) correction and passive autonomous tracking are not supplied.
- **Status:** STANDARD FEEDBACK/STATISTICAL TOOLS RETAINED; BOUNDED EXPLORATORY CLOSED-LOOP MODEL TESTED. A finite-count trace improves at a high measurement cost; a lower-count trace fails to beat the fixed-angle comparison. No hardware advantage or foundational debt closure follows.

## EXT-024 — Restoring dynamics, logical invariance and dissipative-protection comparison

- **Native source check:** reused EXT-022's live DAG reconciliation and read canonical REALFIBER O1–O7 again; re-read the actual real reference rotation, trine endpoint, and the constitutive-divisor extension section 4 equations (17)–(19). Read the cited DIS chapter 10 material response and chapter 12 stored-energy passages. These already provide the conditional energy/damping framework; they do not choose a native restoring potential for the angle. No corpus-wide absence claim is made.
- **Retained tools and exact uses:** EXT-004's passive energy accounting gives displacement work, decay and moving-environment power. EXT-022's finite-dimensional operator/unitary calculus gives the bright projector `H_0^2/a^2` and the parameter-derivative force identity. The nominal trine force impulse is `2Y` for a reciprocal physical angle coordinate. The positive penalty is an algebraic candidate with new couplings, not an admissible-control implementation. Logical/restoring factorization is a stated sufficient model condition; the native dynamics have not been shown to realize it.
- **External comparison only:** [Mirrahimi et al.](https://arxiv.org/abs/1312.2017), primary PDF abstract and introductory encoding discussion, and [Gertler et al.](https://arxiv.org/abs/2004.09322), primary abstract. These concern preserving encoded information through engineered dissipation. No cat-code construction, Lindblad coefficients, fidelity figure or autonomous-correction theorem is adopted for RH-1. Continuous drives in the Gertler abstract are retained as a reminder to account for supplied power, not as evidence of equilibrium passive protection.
- **Boundary:** a stable well requires physical storage and a sink; the moving minimum must emerge from a coupling to the actual environment. The original dark plane lies between bright energies `-a,+a`, so an energy gap around zero does not make it a ground subspace. Making the angle movable produces logical-state-dependent back-action in the tested conjugate-force model. Neither observation rules out a new engineered joint instrument.
- **Status:** CONDITIONAL RESTORING CRITERION AND NATIVE OPERATOR DIAGNOSTICS DERIVED; ACTUAL RESTORING COUPLING, RESERVOIR AND DATA-PRESERVING AUTOTUNE OPEN. The [analysis](2026-09-06-RH-1-restoring-dynamics.md) supersedes measured angle correction as the user-directed build target. No passing artificial-spring simulation or foundational debt closure is claimed.

## EXT-025 — Native Cella three-channel DSP constraint and restoring response

- **Internal reuse, not a new external import:** user identifies the trine as DSP and `F=0` as the constraint surface. The [local construction](2026-09-06-RH-1-cella-constraint-surface.md) instantiates DSP as the three chronological loop angles and derives `F_0=D-S+P` from the actual reflection product. This angle chart is explicitly a model realization, not an inferred universal role assignment.
- **Backing proofs checked:** live DAG revision `575f1e672b44c5303a308fa04fef07fa46d61f1f186a65fad1396507d850871b`; `U-0039` Three-Channel Strong Spec and `U-0600` Canonical Invariant Reduction read with matching recorded/source hashes. Read the Three-Channel Extension, active Role Curvature Correction, actual RE-2 trine product, and BR-8 storage/constraint proof; reuse the already inspected DIS material and stored-energy passages. The canonical engine `reference_lift.py` referee was read and used directly. The DAG remains unchanged.
- **Exact use:** the signed Hessian split supplies `(kappa_c,kappa_s,kappa_int)` on the regular DSP surface. The reduction/gauge theorem requires fixed chart and defining-function provenance. EXT-004/007/022 retain passive response, local differential calculus, polar decomposition and numerical integration. Symbolic differentiation and standard scalar root finding are verification tools; the latter is not an autonomous controller.
- **Derived extension and condition:** nominal `g=(1,-1,1)`, `H_F=0`, all curvature channels zero. The admissible common-angle direction `a=(1,1,1)` obeys `a dot g=1`. For the declared constitutive choice `E=k F^2/2`, `M=eta*a*a^T`, the exact decay identity is `F_dot=-eta*k*(a dot g)^2*F` at fixed environment. Curvature remains computed from signed `F`, not from singular `F^2` or infidelity. The constitutive law is a physical model input still requiring a reference/material realization; it is not derived from the bare quantum Hamiltonian.
- **Numerical scope:** perturbed signed Y-component surfaces are computed from the existing surviving-map polar. Cella's exact referee evaluates finite-difference jets, whose numerical status is retained. Three step sizes check convergence; full metrics retain erasure and other logical error components. No native typed-jet or substrate-promotion claim follows from decimal rationalization.
- **Status:** NATIVE DSP ANGLE-CHART CLOSURE, CHANNEL ACCOUNT AND CONDITIONAL RESTORING LAW DERIVED; PHYSICAL AUTOTUNING COUPLING STILL OPEN. The user-directed surface model supersedes the need to pursue the separate operator-square confinement candidate for this task. Provenance is preserved and no foundational debt is closed by numerical checks.

## EXT-026 — Equilibrium reference coupling and the cost of a moving rest point

- **Native basis checked first:** live DAG revision `575f1e672b44c5303a308fa04fef07fa46d61f1f186a65fad1396507d850871b` still validates with 2300 nodes and 4335 edges. Reused the REALFIBER/trine and canonical DSP source proofs already read for EXT-022/025. Re-read the actual propagator, one-angle pullback, BR-8 storage proof, predictive-state closure boundary and DIS chapters 10/12 on conditioned resistance and storage. These support declared constitutive response; none selects the new spin/spring Hamiltonian. Search misses are not a corpus-wide absence claim.
- **Retained standard mathematics and primary source:** [de Voogd, Wagenaar and Oosterkamp, Dissipation and resonance frequency shift of a resonator magnetically coupled to a semiclassical spin (2017)](https://www.nature.com/articles/srep42239), Basic principles, Susceptibility and the generalization to two-level systems. Retain reciprocal coordinate-dependent reference coupling, canonical equilibrium and the need to include reference response/relaxation. The site's equations are partly image-rendered; no unparsed coefficient is imported. The particular two-level partition function, force, strict-convexity bound and native matching coefficient are derived from the local specified Hamiltonian. Elementary Gaussian integration and the equilibrium Fokker–Planck balance are retained standard tools. No external code, device parameters or performance numbers are copied.
- **Exact use and assumptions:** [the passive-reference trial](2026-09-06-RH-1-passive-reference.md) adds `sum[Omega X+(delta_ref-gs)Z]/2` and `N k0 s^2/2`; it declares matched field coupling, independent references, fast Gibbs relaxation, a slow damped angle and frozen nominal small-signal calibration. The resulting minimum matches the native signed constraint only to first order. Thermal channel averages retain surviving and erasure CP maps; a separate laboratory-frame calculation includes deterministic within-word motion. Their combination is not a stochastic joint-device simulation.
- **Comparison only:** [Prados, Bonilla and Carpio, Phase transitions in a mechanical system coupled to Glauber spins](https://arxiv.org/abs/1004.3215), abstract. Its spin-chain dynamics and transition results are not adopted.
- **Status:** STANDARD EQUILIBRIUM/RESPONSE TOOLS RETAINED; FIRST CONDITIONAL PHYSICAL-COUPLING CANDIDATE TESTED. Thermal angle noise and native logical torque rule out exact arbitrary-data preservation at finite reference variance; the smaller tested reference worsens the conditional gate. Physical transduction, common-field accuracy, microscopic bath/noise, repeated correlated use and complete energy/resource cost remain construction debts. Neither a good mean angle nor passing numerical checks closes them.

## Search receipts

### SR-001 — Horizon and comparison work, backfilled on 2026-09-05

Sources actually cited/read in the retained work: Fontanella (EXT-001), Hayward (EXT-002), Anco (EXT-003), Hehl (EXT-006). Exact earlier search queries are unavailable in the current working record. This receipt is supported by the source links in the project documents, not a reconstruction of unseen browsing history.

### SR-002 — Load/transport continuation, 2026-09-05

Queries:

- `site.vu.nl van der Schaft port Hamiltonian passivity effort flow positive semidefinite`
- `site:arxiv.org port Hamiltonian systems dissipative positive semidefinite skew symmetric energy balance`

Read: Bartel et al., abstract and section 1 of version 1 (EXT-004); Hayward's PDF reopened for null focusing (EXT-005). The attempted Bartel version-2 HTML address returned 404; version 1 was then read successfully. Other returned candidates were not adopted. No external code was copied.
...
### SR-010 — RH-1 opportunity assessment, 2026-09-06

- Query: `Gregoratti Werner Quantum Lost and Found environment assisted correction quantum 2003`.
- Source actually read: [Gregoratti–Werner, Quantum Lost and Found](https://arxiv.org/abs/quant-ph/0209025), primary abstract only. Used as comparison: accessible environment measurements already belong to the standard correction problem. No theorem from the abstract, hardware construction or originality claim was adopted.
- Native basis: the REALFIBER/FD/SPLIT backing sources already inspected for EXT-022; re-read the actual native code-plane rotation and CP-1 reflection construction, and the RH-1 results/receipts. No new corpus gap is claimed. Existing Kraus completeness and rectangular polar decomposition give the coherent stacking calculation; existing real angle controls give the calibrated compensation probe.
- [Opportunity assessment](2026-09-06-RH-1-opportunities.md) records exact uses, numerical pilots and failure boundaries. New physical proposal: retain coherence across temporal leakage modes. This is not the tested per-dump instrument. Its fixed-calibration gain sweep retains leakage outside the calibrated recovery range. The angle probe keeps the original detuning and fidelity target, changes only commanded native angles, and does not claim blind correction or a completed estimator.

### SR-011 — Gimballed native-angle feasibility, 2026-09-06

- Exact query: `qubit real time feedback frequency drift tracking Ramsey measurements calibration primary paper`.
- Source opened/read: [Vepsäläinen et al. (2022)](https://www.nature.com/articles/s41467-022-29287-4), introduction and Results through equations (1)–(2) plus the sampling/bandwidth paragraph. Search excerpts also returned the authors' PMC copy, Delbecq et al.'s Hamiltonian-estimation experiment, a long-term frequency-tracking paper and a deterministic-sensing paper. Those other results were not opened or adopted.
- Adoption is limited to the standard feedback architecture and statistical scope in EXT-023. The native actuator and paired signed measurement law are evaluated directly in `experiments/2026-09-06/rh1_gimbal_probe.py`; no external code or device-performance number was copied. The unknown plant parameters feed the simulated experiment, while the controller receives only the retained command and probe contrast.
- The [assessment](2026-09-06-RH-1-opportunities.md#a-gimballed-angle-feedback-feasibility) and [receipt](rh1-gimbal-probe.json) distinguish ideal expected readout from finite-count trajectories and expose the very large reference cost. The whole-word hold rule preserves the existing relative geometry; continuous within-word motion introduces an additional rotating-frame term and is not tested here.

### SR-012 — Restoration as a stable rest configuration, 2026-09-06

- Exact queries: `Mirrahimi dynamically protected cat qubits engineered dissipation two dimensional manifold 2014 arxiv`; `autonomous quantum error correction logical qubit engineered dissipation Gertler 2021 Nature`.
- Sources actually opened/read: `https://arxiv.org/pdf/1312.2017`, abstract and introductory encoding discussion; `https://arxiv.org/abs/2004.09322`, abstract. Further PDF text finds returned internal errors, and opening later PDF lines timed out; later sections were not read. Other search-result snippets, reviews and mirrors were not adopted.
- Comparison and retained mathematics are scoped in EXT-024. Native source proofs and DIS storage/constitution were checked as above. No external simulation code, physical reservoir, optimal angle or potential was imported. The analytical native torque and spectrum were checked numerically using the existing propagators; the future storage law remains to be physically constructed.

### Internal receipt — Trine as DSP and Cella three-channel constraint, 2026-09-06

- No online search or new external source. Live DAG queries: `three-channel`, `DSP`, and `Directive`; the first was narrowed to paper/definition/formula/theorem types. The exact DSP substring also matched an unrelated processor abbreviation, so the Directive/Substrate/Product role definitions and actual source documents supplied the interpretation.
- Canonical backing sources and live hash verification are recorded in EXT-025. Local searches covered the canonical topic library and `engine/src/cella`, locating `Three_Channel_KG_Strong_Spec.md`, the extension and role/gauge correction proofs, and `reference_lift.py:three_channel_referee`. The existing native trine and Cella response proofs supply the extension; no keyword miss was treated as absence.
- The [runner](../rh1_cella_surface.py), [receipt](rh1-cella-surface.json) and [derivation](2026-09-06-RH-1-cella-constraint-surface.md) retain the chosen ordered-angle realization, nominal exact arithmetic, numerical perturbed jets, and the distinction between a proved constitutive identity and a realized physical force.

### SR-013 — Simplifying the restoring construction, 2026-09-06

- No search-engine query. Reopened [Bartel et al., Port-Hamiltonian Systems Modelling in Electrical Engineering](https://arxiv.org/html/2301.02024v1), section 1, equations (1)–(2) and the environment input/output energy balance. Reused the standard passive structure already retained in EXT-004; no new external result, device or control architecture was adopted.
- Read the current native DSP constraint and restoring-dynamics notes; reused the saved derivative receipt to check the pullback slope along the sole allowed actuator. The recommendation reduces the existing declared model to one coordinate and moves attention to the physical restoring coupling. It does not infer that a static spring can follow an environmentally shifted operating point.

### SR-014 — First passive-reference coupling, 2026-09-06

- Native DAG queries: `constitutive`, `thermal`, `susceptibility`, `fluctuation`. Returned diagnostic/channel claims were not treated as thermal-control theorems. Backing proof and DIS reuse is recorded in EXT-026; the canonical source paths were used, not unresolved topic symlinks.
- Exact online queries: `spin mechanical coupling equilibrium spin susceptibility spring constant thermal noise two level system mechanical oscillator`; `site:arxiv.org spin mechanical oscillator magnetic field gradient spin thermal equilibrium spring softening`.
- Opened/read: `https://www.nature.com/articles/srep42239`, the primary article's Basic principles and Susceptibility sections, two-level-system generalization, and discussion of semiclassical/relaxation limits; `https://arxiv.org/abs/1004.3215`, primary abstract. The Nature endpoint added a cookie-related query string but returned article text. Equation images were not used as imported numerical formulas. Other results, including entropic-spring, driven-actuator, optomechanical and torsional-magnet snippets, were not adopted. Exact uses are EXT-026.
- The [pre-run plan](superpowers/plans/2026-09-06-rh1-passive-reference.md) fixes the candidate, calibration point and parameter tests. The [receipt](rh1-passive-reference.json) records reference size, temperature, thermal CP channels, tracking failures, mechanical free-energy accounting and source hashes. No native Hamiltonian, original designer prediction, Cella graph or physical QEC instrument was changed.
