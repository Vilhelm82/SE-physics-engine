...
- **Native source check for the continuation:** queried the live Cella DAG for REALFIBER, found GFL:thm:realfiber, and read its canonical REALFIBER_THEOREM.md proof, O1–O7. Existing CP/RE derivations supply the primitive and response machinery. This source check was repeated for SD-1–6. No further online search or external mathematical import was made for CM-1–5 or SD-1–6. The interval certificates use elementary contraction of a map on a closed cube, with the maps and bounds supplied explicitly.
- **Scope and remaining work:** the thirteen-block controls cancel logical second order but retain leakage at that order; the general construction cancels the whole quadratic logarithm and preserves the parent's first-order correction scope. All comparisons require the stated repeatable controls and noise model. Next derive and cancel the short word's independent leakage coefficients, controlling gain sensitivity and targeting the prior echo's duration budget. Historical originality and hardware advantage remain separate comparisons.
- **Status:** STANDARD FINITE-GATE CORRECTION RETAINED; CERTIFIED LOGICAL DETUNING CANCELLATION, FULL QUADRATIC NOISE-LOG CANCELLATION, ARBITRARY-ORDER NATIVE RESPONSE AND HIGHER TEMPORAL REJECTION DERIVED.
