...
- **DERIVED** — reached from the primitives / the potential ratio today or earlier, runner cited.
...
|---|---|---|---|
| c | Unruh | DERIVED-given-KMS | DYN-2 d1: EQ-1 + derived local acceleration recover ħa/(2πk_Bc) at the sonic surface r = r_s. No ground (unobserved). LOCK-3 merged into LOCK-2 |
...
|---|---|---|---|---|
| ∅ | our face: light waves in bound orbits, curved spacetime | established | **GROUND + DERIVED** | PPN β = γ = 1 read off the potential ratio output; Mercury 42.9807″ two paths; deflection 1.7512″ (DYN-1 16/16) |
| G | redshift, Shapiro, escaping GW | named | **GROUND + DERIVED** | first-order lapse −2U from the potential ratio (DYN-1 d3); redshift measured to few×10⁻⁵; Cassini γ; GW detected |
| H | near-horizon greybody filter (G-2) | named | **THEORY**, with a DERIVED geometric-optics limit | full greybody unmeasured. The geometric-optics capture edge — light ring at 3r_s/2, escape cone sin ψ = (3√3/2)ηN³, its fold at P₂(cos θ)=0 — is DERIVED from the potential ratio (DYN-1 f1 + 09-07 chat). EHT shadow consistent at 10–17 % |
...
geometric-optics edge of H is DERIVED. All three came off the same tool — the potential ratio with the horizon as sonic surface.
...
pole. The model owes *one* derivation, not two: how light tunnels at the sonic surface r = r_s. Everything the model has said about
...
5. Runner list: append `prim_dyn1_lapse_from_potential.py` (16/16), `prim_eq1_tolman_from_lapse.py` (12/12),
   `prim_dyn2_horizon_period.py` (12/12) — after workstation re-run.
