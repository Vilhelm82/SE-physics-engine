...
print("=== F-7  (Will's sharpening) V is the only primitive deck-odd generator; the seat supplies an even gain ===")
coefs = []; okE = True
for _ in range(300):
    av = [unit(v) for v in rngn.normal(size=(3, 3))]
    A = (np.cross(av[0], av[1]) + np.cross(av[1], av[2]) + np.cross(av[2], av[0]))/2; nn = A/np.linalg.norm(A); Aabs = np.linalg.norm(A)
    Vn = np.dot(av[0], np.cross(av[1], av[2])); hn = Vn/(2*Aabs)
    ai, aj, ak = av[0], av[1], av[2]; En = np.dot(ak, np.cross(nn, aj - ai))
    coef = (En/(2*Aabs) - 1)/(2*(1 - hn**2))                                         # odd part = V * coef  (F-3a, eps_kij = +1)
    coefs.append(coef)
    # the coefficient is deck-EVEN: recompute it on the deck image (a -> -a): E -> E (both pseudovector-dot-vector-cross terms even), |A| even, h^2 even
    avt = [-v for v in av]; At = (np.cross(avt[0], avt[1]) + np.cross(avt[1], avt[2]) + np.cross(avt[2], avt[0]))/2; nt = At/np.linalg.norm(At)
    Et = np.dot(avt[2], np.cross(nt, avt[1] - avt[0])); ht = np.dot(avt[0], np.cross(avt[1], avt[2]))/(2*np.linalg.norm(At))
    coef_t = (Et/(2*np.linalg.norm(At)) - 1)/(2*(1 - ht**2))
    okE &= abs(coef - coef_t) < 1e-10
check("F-7a the coefficient multiplying V in the odd part is deck-EVEN (unchanged on the deck image, 300 frames): the observer-axis geometry"
      " contributes no orientation datum of its own", okE)
check("F-7b that even coefficient takes BOTH signs across the cell: the seat changes the gain and can flip the measured sign RELATIVE to V,"
      " while the odd square class stays generated by the plain triple product", min(coefs) < 0 < max(coefs),
      f"coefficient range {min(coefs):.3f} .. {max(coefs):.3f}; negative on {sum(c < 0 for c in coefs)}/300")

n_pass = sum(CH); print(f"\n{n_pass}/{len(CH)} checks passed (with F-7)")
