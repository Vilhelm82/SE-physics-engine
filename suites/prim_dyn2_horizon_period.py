# prim_dyn2_horizon_period.py  --  DYN-2 / LOCK-2: the thermal scale at the sonic surface r = r_s, run FORWARD.
#
...
# GOAL: derive (b) and (c) from the potential ratio so that ONLY (a) remains, and label (a) as the residue.
#
# INPUTS:
#   DYN-1  N^2 = W_in/(W_in+W_out) from two spreading resistances (recomputed; the OUTPUT lapse, no pinning).   [DERIVED 09-07]
#   T4b    the Wick face of c: vector reading has period 2 pi, the cover is reached as Sym^2 (4 pi).          [DERIVED, prim_t4]
#   SNGL    a potential is single-valued around any closed loop through a node (single-valuedness): a phase that winds
#          around the sonic surface r = r_s must return to itself. This is the network form of "no conical defect".        [GROUND: single-valuedness]
...
# KILLS: (K1) the lapse must be LINEAR in proper distance at the sonic surface r = r_s with a nonzero slope; if the leading power
...
print("=== DYN-2a: the lapse near the sonic surface r = r_s, in PROPER distance (from the potential ratio) ===")
RL = sp.integrate(1/r**2, (r, rs, r)); RS = sp.integrate(1/r**2, (r, r, sp.oo))
A = sp.simplify(RL/(RL+RS)); N = sp.sqrt(A)
def unpw(e):   # the runner's domain is r > r_s (outside the sonic surface r = r_s); take that branch of any Piecewise
    e = sp.piecewise_fold(e)
    return e.args[0][0] if isinstance(e, sp.Piecewise) else e
rho_of_r = unpw(sp.integrate(1/N, (r, rs, r)))         # proper distance from the sonic surface r = r_s, domain r > r_s
print("  N^2 =", A)
...
print("  N(rho) =", N_rho)
kappa = sp.simplify(sp.limit((N/rho_of_r).subs(r, rs+x), x, 0, '+'))   # exact slope at the sonic surface r = r_s, no expansion
print("  kappa = lim N/rho at the sonic surface r = r_s =", kappa)
check("a1", sp.simplify(kappa - 1/(2*rs))==0, "kappa = 1/(2 r_s): the sonic surface r = r_s's slope is the twist scale, DERIVED from the potential ratio (was the pinning's tail)")
lead = sp.Poly(N_rho, rho).monoms()[-1][0]
check("a2", lead==1 and N_rho.coeff(rho,2)==0, f"N = kappa rho + O(rho^3), leading power {lead}, no rho^2 term: Rindler at the sonic surface r = r_s (K1 passes)")
# the static seat's proper acceleration from the potential ratio: a = c^2 dN/dr ; redshifted acceleration a N -> c^2 kappa
a_static = sp.simplify(c**2*sp.diff(N, r))
aN_lim = sp.simplify(sp.limit(a_static*N, r, rs, '+'))
print("  a_static(r) =", a_static, "   a*N at the sonic surface r = r_s =", aN_lim)
check("a3", sp.simplify(aN_lim - c**2/(2*rs))==0, "redshifted acceleration c^2/(2 r_s) recovered as OUTPUT (T4 had it as an input)")

print("=== DYN-2b: the Euclidean loop about the sonic surface r = r_s, and its period ===")
# near the sonic surface r = r_s the seat's line element is  -N^2 c^2 dt^2 + d rho^2  with N = kappa rho.
# Wick the seat (T4: the Wick face of c): t -> -i tau.  Then  ds_E^2 = d rho^2 + rho^2 d(c kappa tau)^2 : POLAR coordinates
# about the sonic surface r = r_s with angle theta = c kappa tau.
tau, th = sp.symbols('tau theta', real=True)
dsE = sp.expand((kappa*rho)**2*c**2*tau**2)             # the angular part, symbolic marker
check("b1", sp.simplify((kappa*rho*c)**2 - rho**2*(c*kappa)**2)==0, "Euclidean line element is d rho^2 + rho^2 d(c kappa tau)^2: polar about the sonic surface r = r_s, angle = c kappa tau")
# SNGL: a single-valued phase around the node returns after one full turn: theta has period 2 pi.  T4b: the vector
...
print("  angle period 2 pi (SNGL single-valuedness = T4b vector reading) => tau period beta =", beta)
check("b2", sp.simplify(beta - 4*sp.pi*rs/c)==0, "beta = 4 pi r_s / c : the period at the sonic surface r = r_s, DERIVED (regularity is SNGL, scale is the potential ratio's)")
...
check("d1", sp.simplify(sp.limit(ratio, r, rs, '+') - 1)==0, "at the sonic surface r = r_s T_local -> hbar a/(2 pi k_B c): the Unruh form is RECOVERED from EQ-1 + the potential ratio; LOCK-3 = LOCK-2 = the one KMS line (K3 passes)")
check("d2", sp.simplify(sp.limit(T_local, r, sp.oo) - T_ref)==0, "the reference node reads T_ref: Hawking's temperature IS Tolman's T_inf for a seat in equilibrium with the sonic surface r = r_s")
print("  far from the sonic surface r = r_s the ratio ->", sp.limit(ratio, r, sp.oo), ": the static seat's local Unruh form is NOT the Tolman temperature away from the horizon (they agree only at the node)")

print("=== DYN-2e: the dual sheet (involution sigma: eta -> 1/etaped) ===")
...
check("e1", Nd.subs(r,rs)==1, "no zero of the dual lapse at r_s: the dual sheet has NO sonic surface at the horizon")
...
print("TIER: kappa = 1/(2 r_s), the Rindler form, the polar Euclidean loop, and beta = 4 pi r_s/c are DERIVED (potential-ratio + T4b/SNGL).")
