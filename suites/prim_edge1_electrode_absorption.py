# prim_edge1_electrode_absorption.py  --  EDGE-1: the l = 0 edge of the octahedron (light tunnelling AT the electrode), run FORWARD.
#
# LOCK-5 / FOLD-1 left one owed piece of face H: the below-barrier, l = 0, low-frequency behaviour at the electrode.
#
# INPUTS:
#   NS-0   the medium's sound obeys the wave equation of the divider metric ds^2 = -A c^2 dt^2 + dr^2/A + r^2 dOmega^2, A = 1 - r_s/r.
...
#   SHEET-1 d1  at the electrode the cone stalls (v_r = c): no sound travels upstream. Boundary condition: purely INGOING,
#          psi ~ e^{-i omega r*} as r* -> -oo. Circuit readings of this one boundary: DC divider = SHORT (Z_L -> 0, V -> 0);
#          AC = MATCHED (no reflection); from inside = OPEN (no path out). [DERIVED]
...
# KILLS: (K1) |R|^2 + |T|^2 != 1 -> numerics wrong. (K2) sigma(omega->0)/A_H != 1 -> the electrode is not a matched absorber and
#        the DC capture area (GUD-1: pi b_mb^2 = A_H) and the AC absorption disagree: the medium is inconsistent at long wavelength.
#        (K3) a decaying r^-3 tail in the regular static l = 2 solution -> nonzero scalar Love number, ECO-like electrode.
...
def transmission(omega, l, rst_min=-30.0, rst_max=None, h=None):
    # integrate psi'' = (V - omega^2) psi from the electrode outward, psi = e^{-i omega r*} there (ingoing, unit amplitude)
...
check("a2", abs(lim - 1.0) < 0.01, "sigma_abs(omega -> 0) = A_H to better than 1 %: the electrode absorbs long waves with exactly its geometric area (K2 passes; DGM/Page recovered as OUTPUT)")
...
print("     Two different physics -- long waves and slow particles -- read the electrode as the same matched load of area A_H.")

print("=== EDGE-1b: the greybody edge completed: face H at the electrode ===")
...
check("b1", abs(T2_lo/(2*om_lo)**2 - 1.0) < 0.05, "|T_0|^2 = 4 (omega r_s)^2 at low frequency: the l = 0 edge is a power law in omega, set by the ELECTRODE's area, while FOLD-1's l >> 1 edge is a Fermi function set by the FOLD's lambda_L. Face H now has both ends")

print("=== EDGE-1c: the static l = 2 response (scalar tidal Love number) ===")
r = sp.symbols('r', positive=True)
A = 1 - 1/r
def static_op(ph):  # l = 2, omega = 0 scalar equation of the divider metric
...
check("c1", sp.limit(Q2, r, 1, '+') in (sp.oo, -sp.oo) and P2.subs(r, 1) == 1, "Q2 is log-singular at the electrode, P2 is finite: regularity (no reflection at omega = 0) selects P2 alone")
ser = sp.series(P2, r, sp.oo, 6).removeO()
check("c2", sp.expand(P2).coeff(r, -3) == 0 and sp.Poly(sp.expand(P2), r).degree() == 2, "the regular branch is a degree-2 POLYNOMIAL: no r^-3 tail. Scalar Love number k_2 = 0 (K3 passes): the electrode does not deform under a static tide, like a black hole and unlike an ECO")
...
print("TIER: DERIVED from the divider metric + the derived ingoing condition; numerics two-checked by flux conservation. The tensor")
