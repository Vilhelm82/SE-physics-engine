# prim_edge4_tidal_friction.py  --  EDGE-4: tidal friction on the rotating sonic surface, run FORWARD (medium side), then compared.
#
# MEDIUM (MAT-2, sonic surface as membrane):
#   EDGE-3A  a static tide Phi_t = (1/2) E_ij x_i x_j deforms the sonic surface: h(n) = -(r_s^3/c^2) E_ij n_i n_j.        [DERIVED]
#   ROT      the sonic surface r = r_s rotates at Omega about z; in the membrane's frame the pattern h rotates at -Omega.               [kinematics]
...
#            charge-transport current: P = Xi_s int |K|^2 dA.                                                            [alternative coefficient, free]
...
#   Damour 1978/82, Thorne-Price-Macdonald 1986 (EXT-036): horizon surface shear viscosity 1/(16 pi G), surface dissipation coefficient Z_0.
...
a, Om, eta_s, Xi_s, eps0, rs, c, M, mp_, b, G = sp.symbols('a Omega eta_s Xi_s epsilon_0 r_s c M m_p b G', positive=True)
...
Jdot_res = -sp.Rational(25,6)*Xi_s*(1/(4*sp.pi))**2*Om*(2*M)**6*S2_eq*(mp_/b**3)**2
R_req = sp.simplify(sp.solve(sp.Eq(Jdot_res, Jdot_P), Xi_s)[0])
print("  resistive alternative: required Xi_s =", R_req, "=", sp.N(R_req,6), " (G=c=1); Damour's horizon resistivity 4 pi =", sp.N(4*sp.pi,6), "(377 Newton); ratio =", sp.N(R_req/(4*sp.pi),6))
...
# static tide: dM/dt = 0; the dissipated power heats the sonic surface r = r_s at temperature T (DYN-2): T dS/dt = P = -Omega J_dot
...
print("RESULT: the rotating sonic surface as a viscous membrane has a tidal torque with Kerr's structure -- -Omega E^2 r_s^6, m^2-weighted,")
...
print("        The O(1) is reported with its conventions named. The dissipation sits where EDGE-3 put it: in the sonic surface r = r_s, not the bulk.")
