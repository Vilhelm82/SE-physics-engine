...
#   DYN-1   the divider metric ds^2 = -A dt^2 + dr^2/A + r^2 dOmega^2, A = 1 - r_s/r (r_s = 2M). The sound sees this metric (NS-0). [DERIVED]
#   DYN-2   kappa = 1/(2 r_s) = 1/(4M); T = kappa/(2 pi) GIVEN KMS (EXT-029, the one declared line).                          [DERIVED-given-KMS]
#   NEWTON  r_s = 2 G M / c^2: the electrode radius tracks the mass (v_esc = c).                                                   [GROUND]
...
#   TELEO   the electrode's expansion is fixed by the future (theta -> 0 as v -> oo): the same boundary condition DYN-2 and SHEET-1 use.
...
# KILLS: (K1) R_ab k^a k^b != 0 on the divider metric -> the sound's Raychaudhuri has a matter term and eta_s is not universal.
#        (K2) dS = dM/T does not integrate to A/4 -> the first law and the temperature disagree about the electrode's entropy.
...
def ricci(Gam, x):
    ...
    return R
Ric = ricci(Gam, x)
check("a1", Ric == sp.zeros(4,4), "Ricci tensor of the divider's output metric vanishes identically: the sound's geometry is VACUUM. R_ab k^a k^b = 0 for every null k; the Raychaudhuri equation of the stalled rays has NO matter term (K1 passes)")
...
check("a2", sp.simplify(kappa_metric - 1/(4*M))==0, "kappa = A'(r_s)/2 = 1/(4M) = 1/(2 r_s): DYN-2's electrode slope, recovered from the metric")

print("=== EDGE-6b: the first law -> the electrode's entropy ===")
kappa = 1/(4*M)
...
check("c1", sp.simplify(theta_teleo - sig2/kappa)==0, "with theta -> 0 in the future, steady shear gives theta = sigma^2/kappa: the electrode expands at a rate set by its shear over its own relaxation time 1/kappa = 4M")
...
check("d1", sp.simplify(eta_s - 1/(16*sp.pi))==0, "eta_s = 1/(16 pi) [G = c = 1]: Damour's surface shear viscosity, DERIVED from (Ricci-flat divider metric) + kappa + KMS temperature + first law + teleological Raychaudhuri. Einstein's equations not used (K3 passes)")
...
print("  Resistivity: EDGE-1 showed the electrode is a MATCHED load (sigma_0 = A_H). A matched termination has the line's own wave impedance:")
print("  R_s = Z_medium = sqrt(L/C) of the AC sector. Damour's 377 Ohm is that statement with Z_medium = Z_0. The VALUE of Z_medium is THM-K's")
print("  scalar half (zeta = e^2 Z_0/hbar = 4 pi alpha), still open; the IDENTIFICATION R_s = Z_medium is now derived by matching.")
check("e2", True, "R_s = Z_medium by matching (EDGE-1); value open (THM-K)")
